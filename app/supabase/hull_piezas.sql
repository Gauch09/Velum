...
create table if not exists public.hull_piezas (
  proyecto_id  text not null references public.hull_proyectos(proyecto_id) on delete cascade,
...
create index if not exists idx_hull_piezas_proyecto on public.hull_piezas(proyecto_id);
alter table public.hull_piezas enable row level security;

insert into public.hull_piezas (proyecto_id, producto, proceso, cantidad, completado) values
  -- 1273 Gran Tortola

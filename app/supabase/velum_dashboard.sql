-- ============================================================================
-- Dashboard de Producción (datos espejados desde el sistema anterior · base access_data)
...
create table if not exists public.velum_proyectos (
...
create table if not exists public.velum_ordenes (
  numero          text primary key,                  -- ej. 'OT-1816'
  proyecto_id     text references public.velum_proyectos(proyecto_id) on delete cascade,
...
create index if not exists idx_velum_ordenes_proyecto on public.velum_ordenes(proyecto_id);

-- 3) Metadatos de la última sincronización (para mostrar "actualizado hace X").
create table if not exists public.velum_sync_meta (
  id              text primary key default 'singleton',
  ultima_sync     timestamptz,
  filas_proyectos integer default 0,
  filas_ordenes   integer default 0
);
insert into public.velum_sync_meta (id) values ('singleton')
  on conflict (id) do nothing;

-- 4) Seguridad: RLS activado. Solo el service role (que usa el dashboard) accede.
alter table public.velum_proyectos enable row level security;
alter table public.velum_ordenes   enable row level security;
alter table public.velum_sync_meta enable row level security;
...
-- ============================================================================
insert into public.velum_proyectos (proyecto_id, nombre, cliente, total_piezas, completadas, prioridad, fecha_entrega, atrasado) values
...
insert into public.velum_ordenes (numero, proyecto_id, proceso, operario, descripcion, piezas, avance, estado) values
...
update public.velum_sync_meta
  set ultima_sync = now(),
      filas_proyectos = (select count(*) from public.velum_proyectos),
      filas_ordenes   = (select count(*) from public.velum_ordenes)
  where id = 'singleton';
